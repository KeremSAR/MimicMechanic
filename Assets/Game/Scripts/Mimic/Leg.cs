...
{
    public class Leg : MonoBehaviour, IPolyCounterPart
    {
        Mimic myMimic;
        ...
        private void Start()
        {
            _mimicStats = GetComponentInParent<MimicStats>();
        }
            ...
            if (!myMimic.killLegs)   
            {
                ...
                if (growTarget == 1 &&
                    Vector3.Distance(new Vector3(myMimic.legPlacerOrigin.x, 0, myMimic.legPlacerOrigin.z),
                        new Vector3(footPosition.x, 0, footPosition.z)) > maxLegDistance && canDie &&
                    myMimic.deployedLegs > myMimic.minimumAnchoredParts)
                    growTarget = 0;
        ...
        private void DrawLegAndProgress()
        {
        ...
        public void UpdateStats()
        {
            ...
            if (level < _maxLevel)
            {
                ...
                minRotSpeed += 1f;
                maxRotSpeed += 10f;
                minOscillationSpeed += 1f;
                maxOscillationSpeed += 6.5f;
                Debug.Log(legMinHeight);
            }*/
        }
    }
}